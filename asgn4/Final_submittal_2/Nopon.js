...
class Cylinder {
  constructor(segments = 10) {
    ...
    this.normalMatrix = new Matrix4(); // ADDED
  ...
  render() {
    ...
    // Wire up variables to the new shader
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Calculate flat normals for lighting!
      const nx = Math.cos((a0+a1)/2);
      const nz = Math.sin((a0+a1)/2);
      const sideNormal = [nx, 0, nz, nx, 0, nz, nx, 0, nz];
      const topNormal = [0, 1, 0, 0, 1, 0, 0, 1, 0];
      const botNormal = [0, -1, 0, 0, -1, 0, 0, -1, 0];
      const fakeUV = [0,0, 0,0, 0,0]; // Dummy UVs since feet are solid colors

      // Side (2 triangles)
      drawTriangle3DUVNormal([x0, 0, z0,   x1, 0, z1,   x1, 1, z1], fakeUV, sideNormal);
      drawTriangle3DUVNormal([x0, 0, z0,   x1, 1, z1,   x0, 1, z0], fakeUV, sideNormal);
      // Bottom cap
      drawTriangle3DUVNormal([cx, 0, cz,   x1, 0, z1,   x0, 0, z0], fakeUV, botNormal);
      // Top cap
      drawTriangle3DUVNormal([cx, 1, cz,   x0, 1, z0,   x1, 1, z1], fakeUV, topNormal);
    }
  }
}
...
class Nopon {
  ...
  // Convenience: draw a cube with a given matrix & color
  _drawCube(m, color) {
    this._cube.matrix = m;
    // Calculate the normal matrix so lighting scales correctly!
    this._cube.normalMatrix.setInverseOf(m).transpose();
    this._cube.color = color;
    // Allow the Normal visualizer button to work on the Nopon
    this._cube.textureNum = g_normalOn ? -3 : -2; 
    this._cube.render();
  }
  ...
  _drawCylinder(m, color) {
    ...
    this._cyl.normalMatrix.setInverseOf(m).transpose();
    this._cyl.color = color;
    this._cyl.textureNum = g_normalOn ? -3 : -2;
    this._cyl.render();
  }
